...
public class WorldChunkStreamer : MonoBehaviour
{
    ...
    // 바닥 프리팹 기본 경로입니다 (Definition이 비었을 때 사용).
    const string DefaultFloorPrefabPath = "Assets/CartoonTown-LowPolyAssets/Prefabs/Floor/Grass/Floor_Grass_Bright.prefab";
    ...
    IEnumerator BootstrapWorldRoutine()
    {
        ...
        SpawnAllChunksAroundPlayerImmediate();
    ...
    void EnsurePlayerGameplayGate()
    {
        ...
        {
            playerTransform.gameObject.AddComponent<PlayerGameplayStartGate>();
        }
    }

    void SpawnAllChunksAroundPlayerImmediate()
    {
    ...
    void DisableLegacyFloorCollidersInScene()
    {
        ...
        for (int i = 0; i < allObjects.Length; i++)
        {
            ...
            if (!target.name.Contains("Floor_Grass"))
            {
                continue;
            }

            PropCollisionLayers.DisableFloorColliders(target);
        }
    }
    ...
    void InitializeStreamer()
    {
        ...
        if (chunkPool == null)
        {
            ...
            if (chunkDefinition != null && chunkDefinition.floorPrefab != null)
            {
                chunkPool.Prewarm(chunkDefinition.floorPrefab, prewarmCount);
            }
    ...
    void RefreshAllChunks()
    {
        ...
        for (int offsetX = -viewRadius; offsetX <= viewRadius; offsetX++)
        {
            ...
            {
                ChunkCoordinate coord = new ChunkCoordinate(center.x + offsetX, center.z + offsetZ);
                neededCoords.Add(coord.ToVector2Int());
            }
        }
        ...
        {
            DespawnChunk(toRemove[i]);
        }

        pendingChunkSpawns.Clear();
        foreach (Vector2Int needed in neededCoords)
        {
    ...
    void SpawnChunk(ChunkCoordinate coordinate)
    {
        ...
        {
            instance.floorObject.name = "Chunk_" + coordinate.x + "_" + coordinate.z;
            PropCollisionLayers.DisableFloorColliders(instance.floorObject);
        }
    ...
    Vector3 ChunkToWorldPosition(ChunkCoordinate coordinate)
    {
        ...
        float worldY = chunkDefinition.floorPositionY;
    ...
    void SpawnManualPropsForChunk(ChunkInstance instance, ChunkCoordinate coordinate)
    {
        ...
        for (int i = 0; i < chunkDefinition.propEntries.Count; i++)
        {
            ...
            Vector3 propWorldPosition = ChunkToWorldPosition(coordinate) + entry.localPosition;
    ...
    void SpawnRandomPropsForChunk(ChunkInstance instance, ChunkCoordinate coordinate)
    {
        ...
        Vector3 chunkOrigin = ChunkToWorldPosition(coordinate);
    ...
    void DespawnChunk(Vector2Int key)
    {
        ...
        if (instance.floorObject != null)
        {
            chunkPool.Return(instance.floorObject, chunkDefinition.floorPrefab);
        }
    ...
    // ChunkDefinition과 floorPrefab이 올바른지 확인합니다.
    void EnsureChunkDefinitionValid()
    {
        ...
        if (chunkDefinition.floorPrefab != null)
        {
            return;
        }
...
#if UNITY_EDITOR
        GameObject loadedPrefab = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>(DefaultFloorPrefabPath);
        if (loadedPrefab != null)
        {
            chunkDefinition.floorPrefab = loadedPrefab;
        }
#endif
    }
...
public class ChunkPool
{
    ...
    public GameObject Get(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
    {
        ...
        {
            instance = (GameObject)Object.Instantiate(prefab, position, rotation, parent);
        }
    ...
    public void Prewarm(GameObject prefab, int count)
    {
        ...
        {
            GameObject instance = (GameObject)Object.Instantiate(prefab, poolRoot.position, Quaternion.identity, poolRoot);
            instance.SetActive(false);
